...
Adafruit_MAX31855 thermocouple1(SCK_PIN, CS_MAX1_PIN, MISO_PIN);
Adafruit_MAX31855 thermocouple2(SCK_PIN, CS_MAX2_PIN, MISO_PIN);

double current_temperature1 = 0.0;
double current_temperature2 = 0.0;
...
void calculate_profile_individual()
{
  ...
  {
    if (i >= profile_time[3])
      temp_reflow_individual[i] = (profile_temp[int(selected_profile)][4] - profile_temp[int(selected_profile)][3]) / (profile_time[4] - profile_time[3]) * (i - profile_time[3]) + profile_temp[int(selected_profile)][3];
    else if (i >= profile_time[2])
      temp_reflow_individual[i] = (profile_temp[int(selected_profile)][3] - profile_temp[int(selected_profile)][2]) / (profile_time[3] - profile_time[2]) * (i - profile_time[2]) + profile_temp[int(selected_profile)][2];
    else if (i >= profile_time[1])
      temp_reflow_individual[i] = (profile_temp[int(selected_profile)][2] - profile_temp[int(selected_profile)][1]) / (profile_time[2] - profile_time[1]) * (i - profile_time[1]) + profile_temp[int(selected_profile)][1];
    else if (i >= profile_time[0])
      temp_reflow_individual[i] = (profile_temp[int(selected_profile)][1] - profile_temp[int(selected_profile)][0]) / (profile_time[1] - profile_time[0]) * (i - profile_time[0]) + profile_temp[int(selected_profile)][0];
  }
}
...
struct box
{
  ...
  void draw(bool fill)
  {
    ...
    if (fill)
      u8g2.drawRBox(left + frame, top + frame, width - (2 * frame), height - (2 * frame), radius);
    else
      u8g2.drawRFrame(left, top, width, height, radius);
  }
  ...
  void print_text(String str, fonts f, box_alignment align, float pos)
  {
    ...
    int x = 0;
    if (align == Center)
      x = left + 0.5 * (width - (f.width * str.length() - 1));
    else if (align == Left)
      x = left + 5;
    else if (align == Right)
      x = left + width - f.width * str.length() - 5;
  ...
  if (!new_page)
  {
    if (pages[current_page].page.is_value)
    {
      ...
      { // set profile page
        ...
        mid_main.print_text(String(*pages[current_page].page.value, 1) + "C", font_s, Center, 0.5);
    }
    else
    {
      if (temp1_error)
...
void setup()
{
  ...
  // store_profile(); //store basic profile 0 on startup for safety. only modify profiles 1+
  read_all_profiles(); // load all profiles
...
void loop()
{
  ...
  if (millis() >= t_thermo + 200)
  {
    ...
    temp1_error = isnan(temp1);
    temp2_error = isnan(temp2);

    if (!temp1_error)
    {
      current_temperature1 = temp1; // random(50, 300);//
    }
    if (!temp2_error)
    {
      current_temperature2 = temp2; // random(50, 300);//thermocouple2.readCelsius();
    }
